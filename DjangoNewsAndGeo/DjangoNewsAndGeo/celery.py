...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'core.settings')# установает значение по умолчанию для среды DJANGO_SETTINGS_MODULE, чтобы Celery знала, как найти проект Django.

app = Celery('core') # создаем экземпляр Celery с именем core и поместили в переменную app.
